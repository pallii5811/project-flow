...
## 2026-09-24 — Batch 7: the only growth channel we can afford is a machine that cuts ten clips a day

**Decision:** An ingested series becomes **social clips at volume** — vertical, captioned, ending on a link to a free episode — through one command, `scripts/make-clips.mjs`, with every decision a pure function in `scripts/lib/clip-rules.mjs`. Four parts, each refusable on its own:

1. **The right to clip comes first.** `socialClipsAllowed: true` plus `socialClipsPermission: { grantedOn, source }`, the same shape `splitPermission` takes before a compilation is cut, checked by ingest (where a refusal costs nothing) and again by make-clips. Cutting a piece out of somebody's film and publishing it under our name on TikTok is a use of their work; the date and where the studio said yes are what an argument gets settled with. A pack of ours is allowed but **labelled** on the clip itself and in every description, so a stand-in can never pass for a licensed drama.
2. **The moment is chosen, never rolled.** Three kinds — the cliffhanger (the last stretch, stopped a beat BEFORE the resolution, which for this format is the strongest unit there is), the cold open of episode 1, and dialogue peaks slid across an episode — each scored from things ffmpeg measured (speech density from the subtitle cues, loudness range from `ebur128` ten times a second, picture cuts from `scdet`, position in the episode, dialogue characters per second) with six named weights that travel in the clip's own record. A start moves onto a picture cut within 1.5 s; an end moves onto the end of a spoken line within 1.2 s. A window is refused, by name, for black, for silence over 35% of it, for reaching the series' own end card, or for starting or ending inside a line — a clip that opens mid-word reads as a broken upload.
3. **The render is measured, not asserted.** 1080×1920, H.264 and AAC, two-pass `loudnorm` to −14 LUFS read back with `ebur128`, a 1.5 s end card, burned-in captions from the episode's own VTT on an **opaque** plate, and a hook line in the first seconds taken from the first thing said inside the clip or the episode's own hook — never composed. Where the text really landed is found by drawing the same subtitles over flat green and locating every pixel that is not green, one frame per text event; the type shrinks until that box is inside the area the platforms' own interface leaves free (bottom 25%, right 15%) and no caption needs a third line.
4. **Volume, rhythm and memory.** Ten a day by default, spread so the same episode is never clipped twice running while another has something left, and `clips/<slug>/clips.json` IS the memory: a second run continues the calendar instead of repeating. Each clip gets a description per platform built from fixed fragments plus the series' own metadata, hashtags from a maintained list, and a link carrying `utm_source`, `utm_medium=clip` and `utm_campaign=clip-<code>`.

**Why:** the owner has no advertising budget and is not getting one. Every app in this format grew by posting 15–40 vertical clips a day across four platforms for months. That is the only channel available, and it is work a person cannot do by hand at that rate — finding the moment, cutting it, burning the subtitles, writing the copy, building the link and remembering what was already posted. Everything except pressing Post is machine work.

**Why the text is measured instead of positioned, measured:** a margin equal to the safe area puts the opaque PLATE outside it by exactly its padding, and shrinking the type does not fix that because the padding does not shrink with it. The first run refused to fit and named the overshoot — 16 px past the top — which is how the bug was found rather than posted.

**Consequences:**

- Checks: `test/clip-rules.test.ts` (62: the rights gate, the ffmpeg text, the end-card detection, snapping, the weights, every refusal, the rhythm, the links, the copy, the ASS the text is drawn with). `npm run proof:clips` (23 cases) builds four one-minute episodes with a picture cut every four seconds, a line every five and a still card for the last three, and runs the real command over them.
- **Measured** on that proof: the clean episode's cliffhanger lands at 16.000–55.000 s, which is the millisecond the rules say; the rendered file's first frame differs from the master's frame at the cut by **0.06/255** and from the frame before it by **30.50/255**, so the cut is frame-accurate; the burned-in text occupies 245–820 × 154–1417 px inside a free area of 65–918 × 154–1440; contrast **9.55:1** at worst over the frames it is really drawn on (AA asks 4.5:1); **−14.3 LUFS** with the true peak under −1 dBFS; 40.52 s and 17.3 MB, inside all four caps. The three defective episodes are refused for their own reasons — `black_in_clip`, `silent_clip`, `cut_mid_line` — and a series with no licence, one claiming a licence without recording it, and one with no subtitles are refused before anything is encoded.
- **Found by writing the proof:** `-ss` on a lavfi source is accepted and silently ignored, so every probe frame came back as frame 0 and every clip measured a contrast of exactly 1.0:1 over a flat colour bed. A measurement that lands on the wrong frame is worse than no measurement, because it has a number. The mask and the frame are now pinned to the same instant on the same grid.
- **Found by the unit tests:** wrapping a caption returned only the first two lines of a three-line one, so the words that did not fit were dropped in silence — half a sentence on screen under the half being spoken. The wrap now never drops a word, and the fitting loop shrinks the type instead, which raises the characters a line holds.
- Cost, measured on this machine: **about 8 s per clip** on the stand-in pack (9.5 s clips) and **18 s** for a 40-second one at 1080×1920 — so a day of ten is **1.5 to 3 minutes** of machine time. The moment selection is cached per episode and per master, so a second day pays only for the rendering.
- Sabotage: four rules broken one at a time and each proven to turn its own check red — the caption drawn into the platform's button row (caught 455 px in, and no clip was produced), the opaque plate lightened to mid grey (9.55:1 became 3.17:1), the render seeking a second past the cut (0.06/255 became 14.87), and the written permission carried into the catalog the browser is built from. Every file restored from a copy and compared with `cmp`.
- `docs/clips.md` is the owner's procedure: the daily routine, what each platform wants and where its link goes, how to read `utm_campaign` back to a clip, the five things to try when a clip does badly, and the rule that comes before all of it.
- `socialClipsPermission` joins the series manifest and stays there: like the producer of record, it never reaches the browser, pinned by a test in `seriesManifest.test.ts` and confirmed on a real export (0 files in `apps/web/out` carry it).
- Not done, and declared: the machine cannot judge whether a moment is good, only how it measures; it cannot see a spoiler; it does not post, and TikTok, Instagram and YouTube all restrict automated posting, so that would be separate work with the accounts — the whole channel — at risk; clips are not drawn with the app's own type, because Manrope and Syne are in the repository as `woff2` only and the FreeType inside ffmpeg cannot open it, so the family really used is read back from libass and recorded on every clip; only the default caption track is used; **no clip has been posted yet**, so nothing here is a claim about what performs.

**Revisit:** after the first two weeks of real posting (the weights are six named numbers and arrival counts per `utm_campaign` are what should move them); when a licensed series is the one being clipped rather than a pack of ours; if a `.ttf` of the product's type enters the repository; when a platform's cap or aspect changes, which is one number each.
