...
const transcript = join(work, "transcript.json");
{
  ...
  // Written through stdout, never with -f: tar reads "C:/…" as a machine to
  // connect to, which is a Windows afternoon nobody needs twice.
  const tarred = spawnSync("tar", ["-c", "-C", staged, "repo", "out"], { maxBuffer: 64 * 1024 * 1024 });
  if (tarred.status !== 0) console.error(`scaleway-proof: tar could not pack what comes back: ${tarred.stderr}`);
  else writeFileSync(collectTar, tarred.stdout);
