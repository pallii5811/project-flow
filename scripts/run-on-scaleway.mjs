 ...
 *
 *   node scripts/run-on-scaleway.mjs --sweep [--older-than-minutes 60] [--force] [--dry-run]
 *
...
async function sweep(api, { olderThanMinutes, force, dryRun }) {
  const servers = await api.listServers({ tag: RUN_TAG });
  const volumes = await api.listVolumes({ tag: RUN_TAG });
  say(
  ...
  const left = await api.listServers({ tag: RUN_TAG });
...
async function sendWorkingCopy(remote) {
  ...
  if (existsSync(records)) {
    const tar = spawn("tar", ["-c", "-C", repoRoot, ".ingest-records"], { stdio: ["ignore", "pipe", "ignore"] });
...
function unpack(tarBuffer) {
  ...
  try {
    const extract = spawnSync("tar", ["-x", "-C", work], { input: tarBuffer });
    if (extract.status !== 0) {
