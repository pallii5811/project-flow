...
export function cloudInit({ publicKey, shutdownMinutes, ffmpegUrl = FFMPEG_URL, ffmpegSha256 = FFMPEG_SHA256, ytDlpUrl = YT_DLP_URL, ytDlpSha256 = YT_DLP_SHA256 }) {
  ...
  const run = [
    ...
    `  echo "cliffies-run: checking R2 for pre-uploaded delivery"`,
    `  R2_DELIVERY_URL="https://$R2_ACCOUNT_ID.r2.cloudflarestorage.com/$R2_BUCKET/deliveries/$CLIFFIES_SLUG/delivery.mp4"`,
    `  if curl -sf -o /dev/null -w "%{http_code}" -H "Range: bytes=0-0" "https://$(echo $MEDIA_BASE_URL | sed 's|https://||')/deliveries/$CLIFFIES_SLUG/delivery.mp4" | grep -q "206\\|200"; then`,
    `    echo "cliffies-run: found pre-uploaded delivery on CDN, downloading from there"`,
    `    curl -fSL -o "${REMOTE.delivery}/delivery.mp4" "https://$(echo $MEDIA_BASE_URL | sed 's|https://||')/deliveries/$CLIFFIES_SLUG/delivery.mp4"`,
    `  else`,
    `    echo "cliffies-run: no pre-uploaded delivery, downloading from YouTube with yt-dlp"`,
    `    yt-dlp --format "bestvideo*+bestaudio/best" --merge-output-format mp4 --extractor-args "youtube:player_client=ios,mweb,android,web" --retries 10 --fragment-retries 15 --retry-sleep fragment:exp=1:20 -o "${REMOTE.delivery}/delivery.mp4" "$CLIFFIES_LINKS" || yt-dlp --format "bestvideo*+bestaudio/best" --merge-output-format mp4 --extractor-args "youtube:player_client=tv,mweb" -o "${REMOTE.delivery}/delivery.mp4" "$CLIFFIES_LINKS" || yt-dlp --format "bestvideo*+bestaudio/best" --merge-output-format mp4 --extractor-args "youtube:player_client=android_vr,web_creator" -o "${REMOTE.delivery}/delivery.mp4" "$CLIFFIES_LINKS"`,
    `  fi`,
    ...
    `  node -e "const fs=require('fs'); const path='${REMOTE.out}/$CLIFFIES_SLUG/$CLIFFIES_SLUG.cuts.json'; const data=JSON.parse(fs.readFileSync(path)); data.confirmed=true; if (data.crop) data.crop.apply=true; fs.writeFileSync(path, JSON.stringify(data, null, 2)); fs.mkdirSync('${REMOTE.repo}/content/series/$CLIFFIES_SLUG', {recursive:true}); fs.copyFileSync(path, '${REMOTE.repo}/content/series/$CLIFFIES_SLUG/cuts.json');"`,
    `  set +e`,
