...
export function cloudInit({ publicKey, shutdownMinutes, ffmpegUrl = FFMPEG_URL, ffmpegSha256 = FFMPEG_SHA256, ytDlpUrl = YT_DLP_URL, ytDlpSha256 = YT_DLP_SHA256 }) {
  ...
  const run = [
    ...
    `  yt-dlp --format "bestvideo*+bestaudio/best" --merge-output-format mp4 --extractor-args "youtube:player_client=android,web" --retries 10 --fragment-retries 15 --retry-sleep fragment:exp=1:20 -o "${REMOTE.delivery}/delivery.mp4" "$CLIFFIES_LINKS"`,
